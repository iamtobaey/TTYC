...
const Gallery = ({ images }) => {
  const [loadedImageNumber, setLoadedImageNumber] = useState(6);
  ...
  return (
    ...
    >
      <Layout>
        <div className='h-[60vh] md:h-[60vh] text-white relative transition-all duration-300 ease-out'>
          <Image
            src={"/assets/gallery/132a.jpg"}
        ...
        <div>
            ...
            <Masonry gutter='20px'>
              {loadedImages.map((image, i) => (
                <Image
                  ...
                  alt={"Event"}
                  priority
                  className='saturate-50 w-[100%] block hover:saturate-100 transition-all duration-500 ease-in-out cursor-pointer'
          ...
          <div className='flex justify-center mt-5 py-10 bg-gray-100 space-x-5 lg:text-lg'>
            <button
              onClick={() => {
                setLoadedImages(images.slice(0, 6));
                setLoadedImageNumber(6);
              }}
              disabled={loadedImageNumber === 6}
...
export async function getStaticProps() {
  const galleryDirectory = path.join(process.cwd(), "public/assets/photo");
  const filenames = fs.readdirSync(galleryDirectory);
  const images = filenames.map((filename) => `/assets/photo/${filename}`);
  return {
    props: {
      images,
    },
